...
--By state
SELECT State, CAST(Start_Date AS date) AS 'Start_Date', CAST(End_Date AS date) AS 'End_Date', MMWR_Year, MMWR_Week, Age_Group, [COVID-19_Deaths], Total_Deaths, ([COVID-19_Deaths]/NULLIF(Total_Deaths, 0))*100 AS 'Percentage_of_COVID_Deaths'
FROM RTDatabase..COVID_Deaths_Age
WHERE State NOT IN ('United States','New York City') 
ORDER BY State, Start_Date

--For whole U.S.
SELECT State, CAST(Start_Date AS date) AS 'Start_Date', CAST(End_Date AS date) AS 'End_Date', MMWR_Year, MMWR_Week, Age_Group, [COVID-19_Deaths], Total_Deaths, [COVID-19_Deaths]/Total_Deaths*100 AS 'Percentage_of_COVID_Deaths'
FROM RTDatabase..COVID_Deaths_Age
...
SELECT State, CAST(Start_Date AS date) AS 'Start_Date', CAST(End_Date AS date) AS 'End_Date', MMWR_Year, MMWR_Week, Age_Group, [COVID-19_Deaths], Total_Deaths, Footnote
FROM RTDatabase..COVID_Deaths_Age
...
SELECT State, CAST(Start_Date AS date) AS 'Start_Date', CAST(End_Date AS date) AS 'End_Date', MMWR_Year, MMWR_Week, SUM([COVID-19_Deaths]) AS 'All_COVID-19_Deaths', SUM(Total_Deaths) AS 'All_Deaths'
FROM RTDatabase..COVID_Deaths_Age
...
SELECT cov.State, cov.MMWR_Year, cov.MMWR_Week, CAST(cov.End_Date AS date) AS End_Date, SUM(cov.Total_Deaths) AS 'All_Deaths', SUM(cov.[COVID-19_Deaths]) AS 'All_COVID-19_Deaths', causes.Septicemia, causes.Malignant_neoplasms, causes.Diabetes_mellitus, causes.Alzheimer_disease, causes.Influenza_and_pneumonia, causes.Chronic_lower_respiratory_diseases, causes.Other_diseases_of_respiratory_system, causes.Nephritis_nephrotic_syndrome_and_nephrosis, causes.Symptoms_signs_and_abnormal_findings_not_elsewhere_classified, causes.Diseases_of_heart, causes.Cerebrovascular_diseases
...
GROUP BY cov.State, cov.MMWR_Year, cov.MMWR_Week, cov.End_Date, causes.Septicemia, causes.Malignant_neoplasms, causes.Diabetes_mellitus, causes.Alzheimer_disease, causes.Influenza_and_pneumonia, causes.Chronic_lower_respiratory_diseases, causes.Other_diseases_of_respiratory_system, causes.Nephritis_nephrotic_syndrome_and_nephrosis, causes.Symptoms_signs_and_abnormal_findings_not_elsewhere_classified, causes.Diseases_of_heart, causes.Cerebrovascular_diseases

--Percentages of causes of death per total deaths using view
SELECT State, MMWR_Year, MMWR_Week, End_Date, All_Deaths, ([All_COVID-19_Deaths]/All_Deaths)*100 AS 'COVID_Death_Percentage', (Septicemia/All_Deaths)*100 AS 'Septicemia_Death_Percentage', (Malignant_neoplasms/All_Deaths)*100 AS 'Malignant_Neoplasms_Death_Percentage', (Diabetes_mellitus/All_Deaths)*100 AS 'DM_Death_Percentage', (Alzheimer_disease/All_Deaths)*100 AS 'Alzheimer_Disease_Death_Percentage', (Influenza_and_pneumonia/All_Deaths)*100 AS 'Flu_and_PNA_Death_Percentage', (Chronic_lower_respiratory_diseases/All_Deaths)*100 AS 'Chronic__Lower_Respiratory_Death_Percentage', (Other_diseases_of_respiratory_system/All_Deaths)*100 AS 'Other_Respiratory_Death_Percentage', (Nephritis_nephrotic_syndrome_and_nephrosis/All_Deaths)*100 AS 'Nephro_Death_Percentage', (Symptoms_signs_and_abnormal_findings_not_elsewhere_classified/All_Deaths)*100 AS 'Deaths_Not_Elsewhere_Classified_Percentage', (Diseases_of_heart/All_Deaths)*100 AS 'Heart_Diseases_Death_Percentage', (Cerebrovascular_diseases/All_Deaths)*100 AS 'Cerebrovascular_Diseases_Death_Percentage'
